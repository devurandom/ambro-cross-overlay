...
db_findver() {
	...
	if [ -d "${ROOT}"/usr/include/db$(db_ver_to_slot "$VER") ]; then
...
db_includedir() {
	if [ $# -eq 0 ]; then
		...
		if [ -d "${ROOT}/usr/include/db${VER}" ]; then
			echo -n "${ROOT}/usr/include/db${VER}"
			return 0
		else
		...
		do
			if VER=$(db_findver "=sys-libs/db-${x}*") &&
			   [ -d "${ROOT}/usr/include/db$(db_ver_to_slot $VER)" ]; then
				echo -n "${ROOT}/usr/include/db$(db_ver_to_slot $VER)"
				return 0
			fi
		done
...
db_libname() {
	if [ $# -eq 0 ]; then
		VER="$(db_findver sys-libs/db)" || return 1
		if [ -e "${ROOT}/usr/$(get_libdir)/libdb-${VER}.so" ]; then
			echo -n "db-${VER}"
			return 0
		else
